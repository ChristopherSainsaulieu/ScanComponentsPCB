...
setup ()
{
  ...
  d1 = cp5.addDropdownList ("Select your Webcam").setPosition (0, 50);

  customize (d1);		// customize the first list
  a =
    cp5.addButton ("Adjust the webcam").setValue (0).setPosition (0,
								  150).
    setSize (310,
	     35).setColorBackground (gris).
...
break; case (ControlP5.ACTION_RELEASED):
case (ControlP5.ACTION_RELEASEDOUTSIDE):
  ...
  b =
    cp5.addButton ("Take a calibration picture").setValue (100).
    setPosition (0, 230).setSize (310,
				  35).setColorBackground (gris).
    addCallback (new CallbackListener ()
		 {
		 public void controlEvent (CallbackEvent theEvent)
...
case (ControlP5.ACTION_PRESSED):
		 ...
		 TakeACalibrationPicture.
		 setTitle ("Take a calibration picture");
		 TakeACalibrationPicture.setLocation (100, 100);
		 //TakeACalibrationPicture.setDefaultCloseOperation(PFrame2.DISPOSE_ON_CLOSE);
TakeACalibrationPicture.setVisible (true); break; case (ControlP5.ACTION_RELEASED):
case (ControlP5.ACTION_RELEASEDOUTSIDE):
		 // println("send MIDI on release, note off.");
		 break;}
		 }		// end of controlEvent
...
// create a new button with name 'take a picture of your card'
  ...
  c =
    cp5.addButton ("Take a picture of your card").setValue (100).
    setPosition (0, 310).setSize (310,
				  35).setColorBackground (gris).
    addCallback (new CallbackListener ()
		 {
		 public void controlEvent (CallbackEvent theEvent)
...
CheckYourCard.setVisible (true); break; case (ControlP5.ACTION_RELEASED):
case (ControlP5.ACTION_RELEASEDOUTSIDE):
		 // println("send MIDI on release, note off.");
		 break;}
		 }		// end of controlEvent
  ...
  d =
    cp5.addButton ("Check your electronic card").setPosition (0,
							      390).
    setSize (310,
	     35).setValue (0).setColorBackground (gris).
    addCallback (new CallbackListener ()
		 {
		 public void controlEvent (CallbackEvent theEvent)
		 {
...
case (ControlP5.ACTION_PRESSED):
		 ...
		 FindYourMissingComponent.
		 setTitle ("Find your missing component");
		 ...
		 FindYourMissingComponent2.
		 setTitle ("Function after loading file");
...
case (ControlP5.ACTION_RELEASEDOUTSIDE):
  ...
  cp5.addButton ("exit").setValue (0).setPosition (1100, 520).setSize (300,
								       35).
    setColorBackground (gris).addCallback (new CallbackListener ()
					   {

					   public void
					   controlEvent (CallbackEvent
							 theEvent)
					   {
...
exit (); break; case (ControlP5.ACTION_RELEASED):
case (ControlP5.ACTION_RELEASEDOUTSIDE):
					   // println("send MIDI on release, note off.");
					   break;}
  ...
  cp5.getController ("Adjust the webcam").getCaptionLabel ().setFont (font).
    toUpperCase (false).setSize (20);
  a.captionLabel ().getStyle ().marginLeft = 4;


  cp5.getController ("Take a calibration picture").getCaptionLabel ().
    setFont (font).toUpperCase (false).setSize (20);
  b.captionLabel ().getStyle ().marginLeft = 4;


  cp5.getController ("Take a picture of your card").getCaptionLabel ().
    setFont (font).toUpperCase (false).setSize (20);
  c.captionLabel ().getStyle ().marginLeft = 4;


  cp5.getController ("Check your electronic card").getCaptionLabel ().
    setFont (font).toUpperCase (false).setSize (20);
  ...
  cp5.getController ("exit").getCaptionLabel ().setFont (font).
    toUpperCase (false).setSize (24);
	  ...
	  if (theEvent.getGroup ().getValue () == j)
	      ...
	      if (testx.compareTo (x) == 0)
		{
		  ...
		  //println(Width+" "+Height);
		       ...
		       && testVirgule.compareTo (virgule) == 0)
		{
...
draw ()
{
  ...
  fill (bleu);
  if (TypeCard.compareTo (noCard) == 0 || TypeCard.compareTo ("5X.jpg") == 0
      && TypeCard.compareTo ("4XC.jpg") == 0
      && TypeCard.compareTo ("4X.jpg") == 0
      && TypeCard.compareTo ("3XC.jpg") == 0
      && TypeCard.compareTo ("3X.jpg") == 0)
    {
      line (400, 85, 600, 85);
      stroke (bleu);
      strokeWeight (4);
  ...
  fill (bleu);
  if (TypeCard.compareTo (noCard) == 0 || TypeCard.compareTo ("5XC.jpg") == 0
      && TypeCard.compareTo ("4XC.jpg") == 0
      && TypeCard.compareTo ("4X.jpg") == 0
      && TypeCard.compareTo ("3XC.jpg") == 0
      && TypeCard.compareTo ("3X.jpg") == 0)
...
public static class TakeACalibrationPicture extends PApplet
{
  ...
  void draw ()
  {
	...
	save = false;
	selectOutput ("click here for select a file to write to:",
		      "fileSelected");
	cam.stop ();
      }
...
public static class FindYourMissingComponent2 extends PApplet
{
  ...
  void draw ()
  {
    ...
    } catch (NullPointerException e)
    {
      ...
      fill (0, 102, 153);
      text ("because you don't have took a picture of another card", 1100,
	    350);
      fill (0, 102, 153);
    }
    try
    ...
    text ("find your missing component", photoInit.width / 2 + 10,
	  photoInit.height / 2 + 20);
...
public static class FindComponentReference extends PApplet
{
  ...
  void setup ()
  {
    ...
    //nameComponents[0]=nomComposant;
      ...
      d3 =
      cp5f.addDropdownList ("List of missing components").setPosition (820,
								       70);
...
//image(opencv.getOutput(), 0, 0);
  ...
  void BDD (float locx, float locy)
  {
	...
	db.query ("SELECT name,URL FROM components where XMIN <= " + locx +
		  " AND XMAX >= " + locx + " AND YMIN >= " + locy +
		  " AND YMAX <= " + locy + ";");
	while (db.next ())
	  {
